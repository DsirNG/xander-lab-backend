...
# --- 7. 启动新容器 ---
echo "正在启动新容器..."
MAIL_PASSWORD="${SPRING_MAIL_PASSWORD:-$RESEND_API_KEY}"
docker run -d \
  --name $CONTAINER_NAME \
  ...
  -e RESEND_API_KEY="$MAIL_PASSWORD" \
  -e SPRING_MAIL_PASSWORD="$MAIL_PASSWORD" \
  -e MAIL_FROM="$MAIL_FROM" \
  -e EMAIL_REMINDER_ENABLED="${EMAIL_REMINDER_ENABLED:-true}" \
  -e EMAIL_REMINDER_POLL_INTERVAL_MS="${EMAIL_REMINDER_POLL_INTERVAL_MS:-1000}" \
